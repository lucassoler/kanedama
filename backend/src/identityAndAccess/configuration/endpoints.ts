...
import { loginController, loginValidator } from "../writes/driving/webserver/loginController";
...
export const identityAndAccessEndpoints= (router: Router, services: Dependencies): void => {
    ...
    router.post('/identity/login', loginValidator(), ExpressMiddlewares.preErrorHandling(), asyncHandler(loginController(services)));
}